//! TLS client/server over any `std.Io` transport, backed by libssl (OpenSSL 3).
//!
//! libssl runs sans-I/O on a pair of memory BIOs; all transport I/O goes
//! through caller-provided `*Io.Reader`/`*Io.Writer`, so any `Io`
//! implementation works, including green-thread schedulers without OS-thread
//! affinity. See `Connection` for the invariants that make this safe.
...
pub const config = struct {
    pub const RootCa = union(enum) {
        /// OpenSSL's default system trust store.
        system,
...
pub const InitError = error{
    /// OpenSSL object creation or configuration failed.
    TlsInitFailure,
    InvalidCertificate,
    InvalidPrivateKey,
    HostTooLong,
...
pub const ClientError = InitError || Connection.HandshakeError;
pub const ServerError = InitError || Connection.HandshakeError;

/// Upgrades a transport reader/writer pair to a TLS connection by performing
/// the client side of the handshake. There are no minimum buffer sizes; size
/// transport buffers for throughput (~16 KB), not correctness.
pub fn client(input: *Io.Reader, output: *Io.Writer, opt: config.Client) ClientError!Connection {
...
pub fn server(input: *Io.Reader, output: *Io.Writer, opt: config.Server) ServerError!Connection {
...
///
/// libssl never performs I/O and never blocks: every `SSL_*` call is a pure
/// in-memory transform between the two BIOs. All actual transport I/O — and
/// therefore every suspension point — happens in this struct's Zig code, with
/// no OpenSSL stack frames live. This makes the connection safe for
/// green-thread `Io` implementations where a fiber may resume on a different
/// OS thread (OpenSSL's error queue is thread-local).
pub const Connection = struct {
    ...
    /// Cause of the most recent failure; message in `errorMessage`.
    err: ?ErrorCause = null,
    ...
    pub const ErrorCause = enum {
        /// TLS protocol error; see `errorMessage`.
        ssl,
        /// Peer certificate chain or host name verification failed; see
        /// `errorMessage`.
        certificate_verify,
    ...
    pub fn handshake(conn: *Connection) HandshakeError!void {
        while (true) {
            c.ERR_clear_error();
    ...
    /// as `error.ReadFailed` with `err == .truncated`.
                ...
                c.SSL_ERROR_WANT_READ => {
                    ...
                    conn.fillIncoming() catch |fill_err| {
                        conn.err = switch (fill_err) {
                            ...
                            error.ReadFailed => .transport_read,
                            error.WriteFailed => .transport_write,
                        };
...
fn initClient(input: *Io.Reader, output: *Io.Writer, opt: config.Client) InitError!Connection {
...
fn initServer(input: *Io.Reader, output: *Io.Writer, opt: config.Server) InitError!Connection {
...
fn commonCtxSetup(ctx: *c.SSL_CTX) InitError!void {
...
fn newSslWithMemBios(ctx: *c.SSL_CTX) InitError!SslParts {
...
fn addTrustedCerts(ctx: *c.SSL_CTX, pem: []const u8) InitError!void {
...
fn useCertKeyPair(ctx: *c.SSL_CTX, kp: config.CertKeyPair) InitError!void {
    {
